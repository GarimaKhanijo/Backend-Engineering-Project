document.addEventListener("DOMContentLoaded", function() {
    ...
    .then(recipes => {
        recipes.forEach(recipe => {
            ...
            likeButton.setAttribute('data-recipe-id', recipe.id); 
...
if (window.location.pathname.includes('profile.html')) {
    ...
    if (token && userEmail) {
        ...
        fetch('/api/recipes')
            .then(response => response.json())
            .then(recipes => {
                const userRecipes = recipes.filter(recipe => recipe.createdBy === JSON.parse(atob(token.split('.')[1])).id);
                ...
                userRecipes.forEach(recipe => {
                    ...
                    recipeDiv.innerHTML = `
                        ...
                        <button class="delete-btn" data-id="${recipe.id}">D</button>
                    `;
                    recipesContainer.appendChild(recipeDiv);   
                });
...
const displayAllRecipes = async () => {
    ...
    recipes.forEach(recipe => {
        ...
        `;
        recipeDiv.onclick = () => window.location.href = `/recipe.html?id=${recipe.id}`;
        recipesContainer.appendChild(recipeDiv);
    });
};
